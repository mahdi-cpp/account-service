...
func ProcessUserJSONFile(inputFilePath, outputDirPath string) error {
	...
	for _, selectUser := range users {
		userIds.Users = append(userIds.Users, UserID{ID: selectUser.ID})
	}

	// Marshal the single selectUser struct back into JSON, with indentation for readability.
	userJSON, err := json.MarshalIndent(userIds, "", "  ")
	if err != nil {
		return fmt.Errorf("failed to marshal selectUser %s to JSON: %w", userIds, err)
	}

	// Construct the output file path using the selectUser's ID as the filename.
	outputFileName := fmt.Sprintf("%s.json", "users")
...
func main() {
	...
	filePath := "./internal/documents/users.json"
	...
	outputDirName := "./internal/test/tmp/output_users_ids"
